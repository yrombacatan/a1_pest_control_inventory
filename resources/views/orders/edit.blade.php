          ...
          <div class="col-12">
            ...
            <div class="card card-primary card-outline">
              <div class="card-header d-flex p-0">
                <h3 class="card-title p-3">Update order to the system. </h3>
              </div><!-- /.card-header -->
              <div class="card-body">
                ...
                @include('adminlte-templates::common.errors')
                    ...
                    {!! Form::model($order, ['route' => ['orders.update', $order->id], 'method' => 'patch', 'id' => 'ordersForm']) !!}
    ...
    $(document).ready(function(){
        ...
        $(".add-row").click(function(){
            ...
            if( $.trim($('#product_id').val()) == '' && $.trim($('#itemqty').val()) == '' ){
                ...
                var item_prc = $("#itemprice").val();

                // manually get data from select items
                // since this variable only change when selectchange function call 
                item_sku = $("select.selectItems").find(":selected").data("sku");
                item_name = $("select.selectItems").find(":selected").data("name");
                // -->

                item_ttl = (itemqty*item_prc);
                var markup = "<tr><td class='text-center'><a href='' class='delete-row btn btn-danger btn-block btn-xs' title='Remove'><i class='fas fa-times'></i></a><input type='hidden' readonly name='item_id[]' value='" + item_id + "'></td><td><input type='text' class='form-control input-borderless' readonly name='item_code[]' value='" + item_sku + "'></td><td><input type='text' class='form-control input-borderless' readonly name='item_name[]' value='" + item_name + "'></td><td class='text-right'><input type='text' class='colUnit text-right form-control input-borderless' readonly name='item_unit[]' value='" + itemunit + "'></td><td class='text-right'><input type='text' class='colPrc text-right form-control input-borderless' readonly name='item_prc[]' value='" + item_prc + "'></td><td class='text-right'><input type='number' class='colQty text-right form-control input-borderless' readonly name='item_qty[]' value='" + itemqty + "'></td><td class='text-right'><input type='number' class='colTtl text-right form-control input-borderless' readonly name='item_ttl_prc[]' value='" + item_ttl.toFixed(2) + "'></td></tr>";
